...
class SimpleNovaSonic:
    ...
    def _initialize_client(self):
        ...
        self.polly_client = boto3.client("polly", region_name=self.CONFIG["polly"]["region"])
    ...
    async def start_session(self):
        ...
        system_prompt = "You are a friendly female assistant. " \
                        "The user and you will engage in a spoken dialog exchanging the transcripts of a natural real-time conversation. " \
                        "Keep your responses short, generally two or three sentences for chatty scenarios. Match the response language with the language of user."
                        ...
                        # Handle text output event
                            ...
                            if (self.role == "ASSISTANT" and self.display_assistant_text):
                                ...
                                response = self.polly_client.synthesize_speech(
                                    Text=text,
                                    VoiceId=self.CONFIG["polly"]["voice_id"],
                                    Engine=self.CONFIG["polly"]["engine"],
                                    LanguageCode=self.CONFIG['polly']['language_code'],
                                    OutputFormat=self.CONFIG['polly']['output_format'],
                                    SampleRate=str(self.CONFIG["polly"]["sample_rate"])
                                )
